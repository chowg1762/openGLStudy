...
unsigned int CoordinateSystem::createTexture(std::string filePath, bool isRGBA){
...
void CoordinateSystem::processInput(GLFWwindow *window){
    if(glfwGetKey(window, GLFW_KEY_ESCAPE)==GLFW_PRESS)
        glfwSetWindowShouldClose(window, true);
}

int CoordinateSystem::createVertexShader(const char *vertexShader){
...
int CoordinateSystem::createFragmentShader(const char *fragmentShader)
{
...
int CoordinateSystem::createProgram(const char *vertexShader, const char *fragmentShader){
    int success;
    char infoLog[512];
...
void CoordinateSystem::createRectangle(){
    /*Rectangle*/
...
void CoordinateSystem::activateTexture(unsigned int textureID, int textureUnit){
    glActiveTexture(GL_TEXTURE0+textureUnit);
    glBindTexture(GL_TEXTURE_2D, textureID);
}

void CoordinateSystem::drawRectangle(unsigned int textureID){
    glBindVertexArray(RECTANGLE_VAO);
...
void CoordinateSystem::destroyRectangle(){
    glDeleteVertexArrays(1, &RECTANGLE_VAO);
    glDeleteBuffers(1,&RECTANGLE_VBO);
    glDeleteBuffers(1,&RECTANGLE_EBO);
}

void CoordinateSystem::setMatrix(unsigned int shaderProgramID){
...
void CoordinateSystem::doTransform(unsigned int shaderProgramID){
    glm::mat4 trans = glm::mat4(1.0f);
    glUniformMatrix4fv(glGetUniformLocation(shaderProgramID, "transformMatrix"), 1 , GL_FALSE, glm::value_ptr(trans));
}

void CoordinateSystem::initMVPMatrix(unsigned int shaderProgramID){
